...
public class ListUtils {
	...
	public static String listToReplaceStr(List<?> objList, String perFieldName, String secFieldName,
										  String sufFieldName){
		List<String> strList = new ArrayList<String>();
		for (Object object : objList) {
			String perStr = null;
			...
			try {
				perStr = (String)PropertyUtils.getProperty(object, perFieldName);
				secStr = (String)PropertyUtils.getProperty(object, secFieldName);
				sufStr = (String)PropertyUtils.getProperty(object, sufFieldName);
			} catch (InvocationTargetException e) {
				e.printStackTrace();
			} catch (NoSuchMethodException e) {
			...
			strList.add(perStr+ " " + secStr + "_" +sufStr);
		}
		return StringUtils.join(strList, ",");
	}
}
