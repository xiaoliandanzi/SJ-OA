		...
		if(null != toolBarUrl && toolBarUrl.size() > 0) {
			...
			for(DataGridToolBarUrl toolBar : toolBarUrl) {
				...
				if(StringUtils.equals("add", toolBar.getType())) {
					...
					//导出
					sbToolBar.append("<button class=\'btn btn-primary\' style=\'margin-left:3px;\' type=\'button\' onclick=exportQh(\'" + this.getName() + "\',\'" + toolBar.getUrl() + "\',\'" + (StringUtils.isEmpty(toolBar.getWidth()) ? "" : toolBar.getWidth()) +"\',\'"+(StringUtils.isEmpty(toolBar.getHeight()) ? "" : toolBar.getHeight())+"\',\'"+(StringUtils.isEmpty(toolBar.getParams()) ? "" : toolBar.getParams())+"\');><i class=\'fa fa-level-up\'></i>&nbsp;导出</button>");
